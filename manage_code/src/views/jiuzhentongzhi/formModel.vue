				...
				<el-row >
					...
					<el-col :span="12">
						<el-form-item label="通知时间" prop="tongzhishijian">
							<el-date-picker
								class="list_date"
								v-model="form.tongzhishijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.tongzhishijian?true:false"
								placeholder="请选择通知时间" />
						</el-form-item>
					</el-col>
					<el-col :span="12">
		...
		if(formType == 'add'){
			...
			for(let x in row){
				if(x=='tongzhibianhao'){
					...
					disabledForm.value.tongzhibianhao = true;
					continue;
				}
				...
				if(x=='tongzhishijian'){
					form.value.tongzhishijian = row[x];
					disabledForm.value.tongzhishijian = true;
					continue;
				}
				if(x=='zhanghao'){
