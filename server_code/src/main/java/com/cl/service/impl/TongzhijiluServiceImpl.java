...
public class TongzhijiluServiceImpl extends ServiceImpl<TongzhijiluDao, TongzhijiluEntity> implements TongzhijiluService {
	...
	@Override
	public List<TongzhijiluView> selectFailedList(Wrapper<TongzhijiluEntity> wrapper) {
		return baseMapper.selectFailedList(wrapper);
	}
	
	@Override
	public boolean handleNotification(Long id, String chulizhuangtai, String chulibeizhu, String chuliren) {
		TongzhijiluEntity entity = this.selectById(id);
		if (entity == null) {
			return false;
		}
		entity.setChulizhuangtai(chulizhuangtai);
		entity.setChulibeizhu(chulibeizhu);
		entity.setChuliren(chuliren);
		entity.setChulishijian(new Date());
	...
	public boolean retryNotification(Long id) {
		...
		// 重置状态为待发送
		entity.setJieshouzhuangtai("0");
		entity.setShibaiyuanyin(null);
		entity.setFasongshijian(null);
		entity.setZuihouzhongshishijian(new Date());
		entity.setZhongshicishu(entity.getZhongshicishu() + 1);
		return this.updateById(entity);
	}
